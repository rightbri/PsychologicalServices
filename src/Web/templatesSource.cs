...
public class SD_PROJECTFILE_V5 : ITemplateClass {
	...
	// cache for v5 relationship infos created from the v2 relation pairs. If a key isn't present for a relation, the opposite is used
	...
	// escapes the name specified and also removes ':' characters. 
	private string GetCleanContainerName(string containerName)
	{
		// this function doesn't remove ':' characters, which can make the project unloadable through xml
		return System.Security.SecurityElement.Escape(containerName).Replace(":", "");
	}
	...
	// to be able to re-pair relations, we've to determine the primary and secondary relation for the v5 relationship info.
	...
	// method which produces the real field definition in case the specified field is a PK field which is in a subtype. In v5 subtypes 
	...
	// gets the relation's full name. It first determines the v5 relationship info for this relation and then returns the full name
	...
	private void PreProcess()
	{
		...
		// now re-pair each two opposite relations into 1 relationshipinfo as v5 uses 1 edge which used on both sides. 
	...
	private string ProduceRelatedFieldName(EntityRelation relation, EntityFieldDefinition fkField)
	{
		...
		{
			// if the related field is in a subtype, the PK of that subtype is removed in v5, it's always inherited from the supertype.
	...
	// Produces for the spcall specified a unique name. This is required because retrieval and action procs are now in the same bucket in v5.
	...
	// info class for storing normal relationship information as needed for v5.
	internal class RelationshipInfo
	{
									  ...
									  EntityRelationType relationshipType, EntityRelation relation, bool startIsPkSide, bool modelOnly)
		{
			...
			if(relation!=null)
			{
				foreach(EntityFieldRelation fieldRelation in relation.FieldRelations)
				{
					// in v5, subtypes inherit the pk from their supertype. So instead of emiting the subtype's pk field, we've to obtain the hierarchy root's
					...
					if(startEntityIsPkSide)
					{
						...
						fkEntity = relation.RelationEndPoint;
					}
					else
					{
						...
						fkEntity = relation.RelationStartPoint;
					}
					if(startIsPkSide)
					{
					...
					// in case of a bug in the relational model, i.e. an FK from a subtype to the supertype over PK-PK, we'll emit the wrong FK field. 
					if(fkEntity.IsSubType && fkField.IsPrimaryKeyField)
					{
						// get proper PK field.
						fkField = GetRealFieldDefinition(fkField);
					}
	...
	// info class for storing indirect relationship information as needed for v5.
...
__outputWriter.Write(GetCleanContainerName(catalog.CatalogName));
		__outputWriter.Write("\">\r\n					<Schemas>\r\n");
		foreach(DictionaryEntry schemaEntry in catalog.DBSchemas)
		{
			DBSchema schema = (DBSchema)schemaEntry.Value;
...
__outputWriter.Write(GetCleanContainerName(target.ContainingSchema.ParentCatalog.CatalogName));
		__outputWriter.Write(":");
			...
			if(!entity.IsSubType)
			{
				...
				if(!string.IsNullOrEmpty(sequenceName))
				{
					if(!systemSequences.Contains(sequenceName))
					{
						...
						sequenceName = string.Format("{0}:{1}:{2}", GetCleanContainerName(target.ContainingSchema.ParentCatalog.CatalogName), target.ContainingSchema.SchemaOwner, realSequenceName);
					}
				}
			}
